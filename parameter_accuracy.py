...
def traverse(param_dir: str):
    ...
    for filename in os.listdir(param_dir):
        if re.match(r"\d{4,4}\.", filename) and filename.endswith('.json'):
            ...
            if (
                mat[1] in visited_file
                or 'gamma' in filename
                or 'beta' in filename
                or 'var' in filename
            ):
                continue
            visited_file.append(mat[1])
            dim_num += s_count
...
if __name__ == '__main__':
    ...
    print(f"Dimension accuracy: {(1 - 4 / dim_num) * 100}%")
    print(f"Parameter accuracy: {(1 - error_param / param_num) * 100}%")
